...
test("MaterialSubject", async (t) => {
  ...
  let mesh = scene.children[0].children[0].children[0] as Mesh<
    BufferGeometry,
    MeshStandardMaterial
  >;
  ...
  texDef1.dispose();
  mesh = scene.children[0].children[0].children[0] as Mesh<
    BufferGeometry,
    MeshStandardMaterial
  >;
...
test("MaterialSubject | dispose", async (t) => {
  ...
  const materialDef = document
    ...
    .setBaseColorTexture(texDef1)
    .setEmissiveTexture(texDef2);
  const meshPrimDef = document
    .createPrimitive()
    .setMode(PrimitiveDef.Mode.TRIANGLES)
    .setMaterial(materialDef);
  const pointsPrimDef = document
    .createPrimitive()
    .setMode(PrimitiveDef.Mode.POINTS)
    .setMaterial(materialDef);
  const meshDef = document
    .createMesh("Mesh")
    .addPrimitive(meshPrimDef)
    .addPrimitive(pointsPrimDef);
  const sceneDef = document
    .createScene("Scene")
    .addChild(document.createNode("Node").setMesh(meshDef));

  const documentView = new DocumentView(document, { imageProvider });
  const scene = documentView.view(sceneDef);
  const [mesh, points] = scene.getObjectByName("Mesh")!.children as [
    Mesh,
    Points,
  ];
  ...
  pointsMaterial.addEventListener("dispose", () =>
    disposed.add(pointsMaterial),
  );
  ...
  const [_, lines] = scene.getObjectByName("Mesh")!.children as [
    unknown,
    LineSegments,
  ];
