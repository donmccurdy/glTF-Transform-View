...
test('NodeSubject', async t => {
	...
	t.deepEqual(node1.quaternion.toArray(), [0, 0, .707, .707], 'node1 → quaternion');
...
test('NodeSubject | update in place', async t => {
	...
	t.truthy(node1.children.some((o) => o.name === 'Mesh.v1'), 'node1.mesh.name');
	t.truthy(node2.children.some((o) => o.name === 'Mesh.v1'), 'node2.mesh.name');
	meshDef.setName('Mesh.v2');
	t.truthy(node1.children.some((o) => o.name === 'Mesh.v2'), 'node1.mesh.name');
	t.truthy(node2.children.some((o) => o.name === 'Mesh.v2'), 'node2.mesh.name');
});
